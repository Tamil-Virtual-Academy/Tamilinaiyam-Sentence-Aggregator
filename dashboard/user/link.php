...
<?php
	...
	$v = 0;
	function recu($user_id,$finals_ids,$li_nks1)
	{
	$r=0;
	$final_id = $finals_ids;
	...
	for($p=0;$p < $r;$p++)
	{
		...
		$html = file_get_contents($link_with_http);
			...
			if(strpos($fetch_link_first,'http')!== false) 
			{
				$fetch_link = mysql_real_escape_string($fetch_link_first);
				$spl_li	=	explode(".",$fetch_link);
			}
			else
			{
				if(preg_match($find,$fetch_link_first) == 1) 
	...
	link:
	$id=mysql_query("SELECT `links_id`,`links` FROM `links` WHERE `links`='$url1'")or die(mysql_error());
	$row = mysql_fetch_array($id);
	$finals_ids	=	$row['links_id'];
	$li_nks        =	explode(".",$row['links']);
	...
	if($count_final != 0)
	{
	recu($user_id,$finals_ids,$li_nks[1]);
